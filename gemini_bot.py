...
owner_rules = {}        # user_id -> [qoidalar ro'yxati]


def business_model_for(bc_id: str):
    """Har bir ega uchun o'z ismi va shaxsiy qoidalari bilan model olish."""
    name = owner_names.get(bc_id, "Xo'jayin")
    owner_id = owner_ids.get(bc_id)
    rules = owner_rules.get(owner_id, [])
...
- Oddiy savollarga bilganingcha yordam berishing mumkin, lekin {name}
  nomidan va'da berma, uning shaxsiy ishlari haqida taxmin qilma."""
    ...
    if rules:
        ...
        prompt += f"""\n\n{name} tomonidan berilgan qo'shimcha shaxsiy ko'rsatmalar
(bular umumiy qoidalardan HAM MUHIMROQ, albatta rioya qil):
{rules_text}"""
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Bu odam biror business ulanishga ega bo'lsa, boshqaruv buyruqlari unga tegishli
    if user_id in owner_bc_ids:
        ...
        # /qoida <matn> — shaxsiy ko'rsatma qo'shish
        if msg.text.strip().lower().startswith("/qoida "):
            rule_text = msg.text.strip()[len("/qoida "):].strip()
            if rule_text:
                owner_rules.setdefault(user_id, []).append(rule_text)
                # Shu egaga tegishli barcha eski suhbatlarni tozalab,
                # keyingi javoblarda yangi qoida hisobga olinsin
                for bc in owner_bc_ids.get(user_id, []):
                    for key in list(chats.keys()):
                        if key.startswith(f"biz_{bc}_"):
                            chats.pop(key, None)
                await msg.reply_text(f"Qoida qo'shildi ✅\n\n«{rule_text}»")
            return

        # /qoidalarim — shaxsiy qoidalar ro'yxatini ko'rsatish
        if text == "/qoidalarim":
            rules = owner_rules.get(user_id, [])
            if not rules:
                await msg.reply_text("Hozircha qoidalar yo'q. Qo'shish: /qoida <matn>")
            else:
                numbered = "\n".join(f"{i+1}. {r}" for i, r in enumerate(rules))
                await msg.reply_text(f"Sizning qoidalaringiz:\n{numbered}")
            return

        # /qoidatozala — barcha shaxsiy qoidalarni o'chirish
        if text == "/qoidatozala":
            owner_rules.pop(user_id, None)
            for bc in owner_bc_ids.get(user_id, []):
                for key in list(chats.keys()):
                    if key.startswith(f"biz_{bc}_"):
                        chats.pop(key, None)
            await msg.reply_text("Barcha shaxsiy qoidalar tozalandi ✅")
            return

        if text == "/yordam":
            await msg.reply_text(
                "Buyruqlar:\n"
                "/off — yordamchini vaqtincha o'chirish\n"
                "/on — yordamchini yoqish\n"
                "/qoida <matn> — shaxsiy ko'rsatma qo'shish "
                "(masalan: /qoida Telefon raqamimni hech kimga berma)\n"
                "/qoidalarim — qo'shilgan qoidalarni ko'rish\n"
                "/qoidatozala — barcha qoidalarni o'chirish"
            )
            return
